    ...
    if(typeof req.session.userInfo !== 'undefined'){
        try {
            ...
            getEmployeeByID(Emp_ID, function(foundEmployee) {
                if (foundEmployee) {
                    getSingleEmployeePhonesFunc(Emp_ID, function(foundPhones) {
            ...
            getEmployeeByID(Emp_ID, function(foundEmployee) {
                if (foundEmployee) {
                    if (Emp_Phone) {
                        ...
                        db.query('DELETE FROM Phone WHERE Emp_ID = ?', [Emp_ID],
                            (error) => {
                                if (error) throw error;
                                res.status(200).redirect('/AdminPage.ejs');
                                alert("Successfully deleted all Emp_Phones for Emp_ID: " + Emp_ID);
                            }
                        );
                    }
                } 
...
function getSingleEmployeePhonesFunc(Emp_ID, callback) {
    db.query('SELECT * FROM Phone WHERE Emp_ID = ?', [Emp_ID],
        (error, results) => {
            if (error) throw error;
            return callback(results);
